...
Let's be honest, modern PHP development has become an art form.

If you're like me, you've spent the last ten years celebrating the rise of our favorite language. With PHP 7, then PHP 8, we finally had the weapons to compete on equal terms with Java or C#. We adopted strict typing (`declare(strict_types=1)`), built hexagonal architectures, sanctified Value Objects, and banished "fuzzy" data.

We built fortresses. And it was necessary for the stability of our PrestaShop stores.

But here's the thing... Artificial Intelligence has entered the room. 🤖
...
Recently, [I threw a small bombshell on LinkedIn](https://nicolas-dabene.fr/articles/2025/12/12/php-loose-typing-ai-asset/) by suggesting that PHP's "Loose Typing" (weak typing), often criticized, could be our best ally against LLM uncertainty. An idea that sparked reactions (rightfully so!) from a colleague who perfectly summarized the "purist" thinking:

> "I'd rather question ≈10 years of PHP improvements [...] than write Value Objects and DTOs that would properly handle casting."

His remark is excellent. It raises the real question that animates us today: **Should we sacrifice the rigor of our software engineering to satisfy AI's whims and merchants' automation needs?**
...
In the PrestaShop ecosystem, stability is key. A type error on a price, and your average cart value collapses or an order goes through at €0. That's why we collectively hardened our code. We want predictability. If a function expects a `float`, it should reject a `string`. It's healthy. It's clean.
...
If you ask the AI for a shipping weight, it might answer:
...
The problem: **If your code is a rigid fortress that throws a `TypeError` at the slightest wrong comma, your automation fails.**
...
This isn't about regressing, but about **cushioning**.

I'm not saying we should remove strict types from our **Core Domain**. Absolutely not! Our tax calculations, our stock management in PrestaShop must remain absolutely rigorous.

However, we need to build a new layer, a **"Decompression Chamber"**.

Imagine your application as a fortified castle:

- **The Throne Room (The Core)**: Here, everything is strict. We speak in Value Objects, in strong types. We don't mess with data.
...
This discussion goes far beyond `declare(strict_types=1)`. It touches on the evolution of our profession itself.
...
Tomorrow's developer (and today's, actually) is one who knows how to combine:

- **Technical robustness** (so the store doesn't break).
- **Operational agility** (so AI can work).

We'll need to learn to "let go" of certain dogmas at our applications' boundaries. Accept that incoming data is dirty, and build the most robust washing machines (intelligent parsers) possible.
...
So, is my colleague right? **Absolutely**. We don't throw 10 years of safety in the trash.
